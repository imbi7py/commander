...
def main():
    ...
    threads = []
    t1 = threading.Thread(target=quickview_send, args=(ip, port, 'pics/realdata/可见光', '北航猛牛', '可见光',))
    threads.append(t1)
    t2 = threading.Thread(target=quickview_send, args=(ip, port, 'pics/realdata/minisar', '北航猛牛', 'minisar',))
    threads.append(t2)
    t3 = threading.Thread(target=quickview_send, args=(ip, port, 'pics/realdata/多光谱', '大疆', '多光谱',))
    threads.append(t3)
    t4 = threading.Thread(target=video_send, args=(ip, port, 'pics/realdata/双波段视频吊舱_可见光', '地理所xxx', '双波段视频吊舱_可见光',))
    threads.append(t4)
