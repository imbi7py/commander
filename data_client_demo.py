...
def main():
    ...
    threads.append(t3)
    t4 = threading.Thread(target=video_send, args=(ip, port, 'pics/realdata/双波段视频吊舱_可见光', '猛牛', '双波段视频吊舱_可见光',))
    threads.append(t4)
    t5 = threading.Thread(target=video_send, args=(ip, port, 'pics/realdata/双波段视频吊舱_红外', '北航猛牛2', '双波段视频吊舱_红外',))
    threads.append(t5)
    t6 = threading.Thread(target=video_send, args=(ip, port, 'pics/realdata/光学相机视频', '北航猛牛2', '光学相机视频',))
    threads.append(t6)
    t7 = threading.Thread(target=quickview_send, args=(ip, port, 'pics/realdata/测绘相机快视图', '猛牛', '测绘相机快视图',))
    threads.append(t7)
    t8 = threading.Thread(target=quickview_send, args=(ip, port, 'pics/realdata/高光谱', '猛牛', '高光谱',))
    threads.append(t8)
    for t in threads:
        t.setDaemon(True)
        t.start()
